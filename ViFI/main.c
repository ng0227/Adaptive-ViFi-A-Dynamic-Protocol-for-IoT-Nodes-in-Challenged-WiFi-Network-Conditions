...
probeNumber=probeNumber+1;        
        ...
        // Each retry will occur after 1s.
        ...
        while (1) {
            ...
            printf("Probe stuffingggggggggggggggggggggggggggggggggggggggggggg");
            ...
            if (err != 0) {  // Scanning failed.
                ...
                driverError=driverError+1;
                    ...
                    // Sleep for 1s before doing next scan.
                    printf("%d retries left. Will retry after 1s. Driver error\n", scan_retries_left);
                    usleep(520000);
                ...
                if (ret < 0) {  // Didn't receive an ACK.
                    ...
                    if (effort == 2) {
                        ...
                        if (ack_retries_left > 0) {
                            nl_socket_free(socket);
                            // Wait for 1 seconds before sending new scan request.
                            printf("%d retries left. Will retry after 1s.\n", ack_retries_left);
                            usleep(520000);
                            socket = init_socket();
                        } else {
                            break;
                   ...
                   successPacketcount=successPacketcount+1;
                        ...
                        log_info(wifi_interface, data_origin_ts, data_origin_ts_usec, data_origin_ts_str,
                            data_tx_ts, data_tx_ts_usec, data_size, 5 - ack_retries_left, 1);
                        printf("ACK received successfully.\n\nWaiting for 1s before sending more data.");
                        fflush(stdout);
                        seq_num += 1;
                        free(ies_data);
                        ack_retries_left = 4;
        ...
        if (scan_retries_left == 0) {
            ...
            printf("\nGoing to sleep for 1s before sending new scan request.\n");
...
////usleep(440000);
	...
	double stopAfterSeconds = 600.0;//x minutes 
	...
	if(duration>stopAfterSeconds){
		...
		printf("\npacketLoss= %d\n",packetLossCount);
		printf("\ndriverError= %d\n",driverError);
			//signal the thread to shutdown;F
