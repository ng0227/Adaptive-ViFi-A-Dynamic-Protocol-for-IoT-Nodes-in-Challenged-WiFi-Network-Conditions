...
int do_probe_stuffing(struct nl_sock *socket, int interface_index,
        int driver_id, int ies_len, unsigned char *ies_data,int channel) {
...
struct nl_msg *freqs;
    ...
    int err;
    int ret;
    int mcid = nl_get_multicast_id(socket, "nl80211", "scan");//headers.h
    // Without this, callback_trigger() won't be called.
    //r subscribing to 	multicast groups
...
return -ENOMEM;
    ...
    nla_put(msg, NL80211_ATTR_IE, ies_len, ies_data);
     ...
     printf("\n\n Probe stuffing:Channel = %d \n\n", channel);
    
     //selective broadcast		
     nla_put_u32(freqs, 1, 5*channel + 2407);
     nla_put_nested(msg,NL80211_ATTR_SCAN_FREQUENCIES, freqs);


    // Scan all SSIDs.
    nla_put(ssids_to_scan, 1, 0, "");// this last argument might be the key , tried to put comma separated ssids but it wasn't working
